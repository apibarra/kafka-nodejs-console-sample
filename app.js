...
if (process.env.VCAP_SERVICES) {
  ...
  if (process.argv.length < 6) {
    console.log(
      "ERROR: It appears the application is running outside of Bluemix but the arguments are incorrect for local mode."
    );
    console.log(
      "\nUsage:\n" +
        "node " +
        process.argv[1] +
        " <kafka_brokers_sasl> <kafka_admin_url> <api_key> <cert_location> [ -consumer | -producer ]\n"
    );
    process.exit(-1);
  }
...
console.log("Connecting the topic " + topicName + " with Admin REST API");
// p adminRestInstance.topics
// p  .create(topicName, 1, 24)
// p  .then(function(response) {
    // If response is an empty object, then the topic was created
// p    if (Object.keys(response).length === 0 && response.constructor === Object)
// p      console.log("Topic " + topicName + " created");
// p    else console.log(response);
// p  })
...
// p  .done(function() {
    ...
    adminRestInstance.topics
      ...
      .then(function(response) {
        ...
        //console.log(response);   Show all the topics
      })
      .fail(function(error) {
        console.log(error);
      })
...
function runLoops() {
  ...
  if ( runConsumer) {
    consumer = ConsumerLoop.buildConsumer(
      Kafka,
      consumer_opts,
      topicName,
      shutdown
